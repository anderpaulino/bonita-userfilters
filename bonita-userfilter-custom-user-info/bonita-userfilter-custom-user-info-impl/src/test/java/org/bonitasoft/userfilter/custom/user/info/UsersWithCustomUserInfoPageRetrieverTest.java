...
public class UsersWithCustomUserInfoPageRetrieverTest {
    ...
    @Test
    public void getMaxPageSize_should_return_the_max_page_size_supplied_in_the_constructor() {
        // when
    ...
    @Test
    public void nextPage_should_retrive_the_next_available_page_and_move_pointer_to_the_following_one() {
        // given
        when(identityAPI.getUserIdsWithCustomUserInfo(CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, false, 0, MAX_PAGE_SIZE))
                .thenReturn(Arrays.asList(1L, 2L));
        when(identityAPI.getUserIdsWithCustomUserInfo(CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, false, MAX_PAGE_SIZE, MAX_PAGE_SIZE)).thenReturn(
                Arrays.asList(3L, 4L));
        when(identityAPI.getUserIdsWithCustomUserInfo(CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, false, 2 * MAX_PAGE_SIZE, MAX_PAGE_SIZE)).thenReturn(
                Arrays.asList(5L));
        when(identityAPI.getUserIdsWithCustomUserInfo(CUSTOM_USER_INFO_NAME, CUSTOM_USER_INFO_VALUE, false, 3 * MAX_PAGE_SIZE, MAX_PAGE_SIZE)).thenReturn(
